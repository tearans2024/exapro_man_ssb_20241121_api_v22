...
Public Class FSalesOrder
    ...
    Public _so_ref_po_oid, _so_sq_ref_oid, _sq_ref_oid As String
    ...
    Public Overrides Sub format_grid()
        ...
        add_column_copy(gv_detail, "Qty SO", "sod_qty", DevExpress.Utils.HorzAlignment.Far, DevExpress.Utils.FormatType.Numeric, "n4")
        add_column_copy(gv_detail, "Qty Outstanding", "sod_qty_open - sod_qty", DevExpress.Utils.HorzAlignment.Far, DevExpress.Utils.FormatType.Numeric, "n4")
    ...
    Public Overrides Function get_sequel() As String
        get_sequel = "SELECT  " _
                    ...
                    & "  so_print_count, " _
                    & "  so_payment_date, " _
                    & "  tran_name,coalesce(so_cons,'N') as so_cons, " _
...
.Command.Open()
                    ...
                    Try
                        ...
                        '    '.Command.CommandType = CommandType.Textea
    ...
    Private Function insert_return(ByVal par_obj As Object, ByVal par_so_oid As String, ByVal par_so_code As String) As Boolean
        ...
        With ds.Tables(0).Rows(row)
            ...
            so_alocated.EditValue = SetBitYNB(.Item("so_manufacture"))
            ...
            For i = 0 To ds_edit.Tables(0).Rows.Count - 1
                If ds_edit.Tables(0).Rows(i).Item("sod_qty") > 0 Then
                    If SetString(ds_edit.Tables(0).Rows(i).Item("sod_sqd_oid").ToString) <> "" Then
                        'update karena ada hubungan antara so dan po antar group perusahaan
                        '.Command.CommandType = CommandType.Text
                        .Command.CommandText = "update sqd_det set sqd_qty_so = coalesce(sqd_qty_so,0) - " + SetDbl(ds_edit.Tables(0).Rows(i).Item("sod_qty").ToString) _
                                             & " where sqd_oid  = " + SetSetring(ds_edit.Tables(0).Rows(i).Item("sod_sqd_oid"))
                        ssqls.Add(.Command.CommandText)
                        .Command.ExecuteNonQuery()
                        '.Command.Parameters.Clear()
                    ...
                    '.Command.CommandType = CommandType.Text
                    .Command.CommandText = "update sqd_det set sqd_qty_booking = coalesce(sqd_qty_booking,0) - " + SetDbl(ds_edit.Tables(0).Rows(i).Item("sod_qty").ToString) _
                                         & " where sqd_oid  = " + SetSetring(ds_edit.Tables(0).Rows(i).Item("sod_sqd_oid"))
                    ssqls.Add(.Command.CommandText)
                    .Command.ExecuteNonQuery()
                    '.Command.Parameters.Clear()

                    'jika preorder tambahkan nilai pre-order
        ...
        '_ptsfr_code = func_coll.get_transaction_number("TR", so_en_id.GetColumnValue("en_code"), "ptsfr_mstr", "ptsfr_code")
        ...
        '                        & SetInteger(so_ptsfr_loc_id.EditValue) & ",  " _
        '                        & SetSetring(so_trans_rmks.Text) & ",  " _
        '                        & SetBitYN(so_booking.Checked) & ",  " _
        '                        & "" & SetDateNTime(master_new.PGSqlConn.CekTanggal) & "" & ",  " _
        '                        & "" & SetDateNTime(master_new.PGSqlConn.CekTanggal) & "" & ",  " _
        '                        & SetSetring(_ptsfr_trn_satatus) & ",  " _
        '                        & SetInteger(so_si_id.EditValue) & ",  " _
        '                        & SetSetring(_so_oid_mstr) & ",  " _
        '                        & SetSetring(_so_sq_ref_oid) & ",  " _
        ...
        '                                & SetInteger(ds_edit.Tables(0).Rows(i).Item("sod_pt_id")) & ",  " _
        '                                & SetDbl(ds_edit.Tables(0).Rows(i).Item("sod_qty")) & ",  " _
        '                                & IIf(so_booking.Checked = True, SetDbl(ds_edit.Tables(0).Rows(i).Item("sod_qty")), "null") & ", " _
        '                                & SetInteger(ds_edit.Tables(0).Rows(i).Item("sod_um")) & ",  " _
        '                                & SetDbl(ds_edit.Tables(0).Rows(i).Item("sod_cost")) & ",  " _
        '                                & "" & SetDateNTime(master_new.PGSqlConn.CekTanggal) & "" & ",  " _
        '                                & SetSetring(ds_edit.Tables(0).Rows(i).Item("sod_rmks").ToString) & ", " _
        ...
        ''.Command.CommandType = CommandType.Text
        '.Command.CommandText = "update sqd_det set sqd_loc_id = " & SetInteger(so_ptsfr_loc_id.EditValue) & " " + _
        '                  " where sqd_oid = '" + ds_edit.Tables(0).Rows(i).Item("sod_sqd_oid").ToString + "'"
        'ssqls.Add(.Command.CommandText)
        '.Command.ExecuteNonQuery()
        ''.Command.Parameters.Clear()
        ...
        ''.Command.CommandType = CommandType.Text
        '.Command.CommandText = "update sq_mstr set sq_close_date = " & SetDateNTime00(master_new.PGSqlConn.CekTanggal) & ", sq_trans_id = 'X' " + _
        '                     " where sq_oid = '" + ds_edit.Tables(0).Rows(i).Item("sod_so_sq_ref_oid").ToString + "'"
        'ssqls.Add(.Command.CommandText)
        '.Command.ExecuteNonQuery()
        ''.Command.Parameters.Clear()
        ...
        '            _en_id = so_en_id.EditValue
        '            _si_id = so_si_id.EditValue
        '            _loc_id = SetInteger(so_ptsfr_loc_to_id.EditValue)
        '            _pt_id = ds_edit.Tables(0).Rows(i).Item("sod_pt_id")
        '            _pt_code = ds_edit.Tables(0).Rows(i).Item("pt_code")
        '            _serial = "''"
        '            _qty = ds_edit.Tables(0).Rows(i).Item("sod_qty")
        ...
        '            _qty = _qty * -1.0
        '            _cost = ds_edit.Tables(0).Rows(i).Item("sod_cost")
        '            _cost_avg = func_coll.get_cost_average("-", _en_id, _si_id, _pt_id, _qty, _cost)
        '            If func_coll.update_invh_mstr(ssqls, par_obj, _tran_id, i_2, _en_id, _ptsfr_code, _ptsfr_oid.ToString, "Transfer Issue", "", _si_id, _loc_id, _pt_id, _qty, _cost, _cost_avg, "", so_date.DateTime) = False Then
        '                Return False
        '            End If
        '        End If
        '    End If
        ...
        '            _si_id = so_si_id.EditValue
        '            If so_booking.Checked = True Then
        '                _loc_id = so_ptsfr_loc_id.EditValue
        '            Else
        '                _loc_id = so_ptsfr_loc_git.EditValue
        '            End If
...
.Command.Open()
    ...
    Private Sub CloseSalesOrderToolStripMenuItem_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles CloseSalesOrderToolStripMenuItem.Click
        Try
            ssqls.Clear()

            If ask("Are you sure you will close the sales order?", "Confirm..", MessageBoxDefaultButton.Button1) = True Then
                Using objinsert As New master_new.CustomCommand
                    With objinsert
.Command.Open()
                        'Dim sqlTran As PgSqlTransaction = .Connection.BeginTransaction
                        Try
                            If so_tran_id.EditValue = "C" Then

                            ElseIf so_tran_id.EditValue = "X" Then

                            ElseIf so_tran_id.EditValue = "D" Then
