...
Public Class FPriceListDetail
    'edited by rana 12/10/2022 menambahkan area jawa - luarjawa untuk pricelist yang berbeda
    'berefek pada sq_mstr dan so_mstr filter pricelist
    ...
    Public Overrides Sub format_grid()
        ...
        add_column_copy(gv_detail_rule, "Area", "area_name", DevExpress.Utils.HorzAlignment.Default)
        ...
        add_column_copy(gv_edit_item, "Taxable", "pid_pt_taxable", DevExpress.Utils.HorzAlignment.Default, DevExpress.Utils.FormatType.Numeric, "n4")
        add_column_copy(gv_edit_item, "Tax Class", "tax_class_name", DevExpress.Utils.HorzAlignment.Default, DevExpress.Utils.FormatType.Numeric, "n4")
        add_column_copy(gv_edit_item, "PPN Type", "pid_pt_ppn_type", DevExpress.Utils.HorzAlignment.Default, DevExpress.Utils.FormatType.Numeric, "n4")
        ...
        add_column(gv_edit_rule, "pidd_area_id", False)
        add_column(gv_edit_rule, "Area", "area_name", DevExpress.Utils.HorzAlignment.Default)
    ...
    Public Overrides Sub load_data_grid_detail()
        ...
        sql = "SELECT pidd_oid, " _
                ...
                & " left outer join public.area_mstr ON pidd_area_id = area_id"
            ...
            Using objcb As New master_new.CustomCommand
                With objcb
                    .SQL = "SELECT  " _
                        ...
                        & "  public.pidd_det " _
                        & "  inner join public.code_mstr on code_id = pidd_payment_type " _
                        & "  left outer join public.area_mstr on pidd_area_id = area_id " _
                        & " where pidd_price = -99"
                    .InitializeCommand()
    ...
    Private Sub gv_edit_rule_DoubleClick(ByVal sender As Object, ByVal e As System.EventArgs) Handles gv_edit_rule.DoubleClick
        ...
        If _col = "area_name" Then
            Dim frm As New FAreaSearch
            frm.set_win(Me)
            frm._row = _row
            'frm._en_id = pi_en_id.EditValue
            frm.type_form = True
                    ...
                    Try
                        ...
                        For i = 0 To ds_edit_rule.Tables(0).Rows.Count - 1
                            ...
                            .Command.CommandText = "INSERT INTO  " _
                                                ...
                                                & SetInteger(ds_edit_rule.Tables(0).Rows(i).Item("pidd_area_id")) & ",  " _
...
.Command.Open()
    ...
    Public Overrides Sub preview()
        ...
        _en_id = ds.Tables(0).Rows(BindingContext(ds.Tables(0)).Position).Item("pi_en_id")
        _type = 10
        _table = "pi_mstr"
        _initial = "pi"
        _ptcode = ds.Tables(0).Rows(BindingContext(ds.Tables(0)).Position).Item("pt_code")
        _code_awal = ds.Tables(0).Rows(BindingContext(ds.Tables(0)).Position).Item("pi_code")
        _code_akhir = ds.Tables(0).Rows(BindingContext(ds.Tables(0)).Position).Item("pi_code")
...
& "  pt_desc1, " _
& "  pidd_area_id, " _
& "  CASE when pidd_area_id = '1' then concat ((select area_name from area_mstr where area_id = '1'), ' ', 'Rp ', pidd_price) end as price_pjw, " _
& "  CASE when pidd_area_id = '2' then concat ((select area_name from area_mstr where area_id = '2'), ' ', 'Rp ', pidd_price) end as price_ljw " _
& "FROM " _
& "  pidd_det " _
& "  INNER JOIN area_mstr ON pidd_area_id = area_id " _
& "  INNER JOIN pid_det ON pidd_pid_oid = pid_oid " _
& "  INNER JOIN pi_mstr ON pid_pi_oid = pi_oid " _
& "  INNER JOIN pt_mstr ON pid_pt_id = pt_id " _
                & "WHERE " _
                & "  pidd_payment_type = '9941' AND  " _
                & "  pi_code ~~* '" + ds.Tables(0).Rows(BindingContext(ds.Tables(0)).Position).Item("pi_code") + "' AND  " _
                & "  pt_code ~~* '" + ds.Tables(0).Rows(BindingContext(ds.Tables(0)).Position).Item("pt_code") + "'"

        '& "pidd_det.so_code ~~* '" + ds.Tables(0).Rows(BindingContext(ds.Tables(0)).Position).Item("so_code") + "'"

        Dim frm As New frmPrintDialog
        frm._ssql = _sql
        frm._report = "rptQRLabelPartnumberPi"
