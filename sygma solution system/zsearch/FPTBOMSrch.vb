...
Public Class FPTBOMSrch
    ...
    Public _obj, _prj_code, _ps_code As String
    ...
    Public Overrides Function get_sequel() As String
        ...
        ElseIf fobject.name = FProductStructure.Name Then
            If _pil = 3 Then
                get_sequel = "SELECT false as pilih, en_mstr.en_desc, pt_oid as oidptbom, pt_en_id as enptbom, pt_id as idptbom, " _
                        & "pt_code as codeptbom, pt_desc1 as descptbom, pt_um, code_name as um_desc, invct_cost " _
                        & "FROM " _
                        & "  public.pt_mstr  " _
                        & " inner join code_mstr on pt_um=code_id " _
                        & " inner join invct_table on pt_id = invct_pt_id " _
                        & "  INNER JOIN en_mstr on pt_en_id=en_id where pt_type='I' and pt_its_id = 1 and pt_en_id in (0," & _en_id & ") "
            Else
                get_sequel = "SELECT en_mstr.en_desc, pt_oid as oidptbom, pt_en_id as enptbom, pt_id as idptbom,  " _
                       & " pt_code as codeptbom, pt_desc1 as descptbom, pt_um, code_name as um_desc, invct_cost " _
                       & "FROM " _
                       ...
                       & " inner join code_mstr on pt_um=code_id " _
                       & " inner join invct_table on pt_id = invct_pt_id " _
                       & "  INNER JOIN en_mstr on pt_en_id=en_id where pt_pm_code='M' and pt_its_id = 1"
        ...
        ElseIf fobject.name = FWODisAssembly.Name Then
            get_sequel = "SELECT DISTINCT en_mstr.en_desc,ps_oid, ps_desc, pt_oid as oidptbom, pt_en_id as enptbom, pt_id as idptbom,  " _
                       & "  pt_code as codeptbom, pt_desc1 as descptbom,invc_si_id," _
                       & "  si_desc,pt_pl_id,wo_ps_id, " _
                       & "  invct_cost " _
                       & "  from public.ps_mstr  " _
                       & "  INNER JOIN pt_mstr on pt_id=ps_pt_bom_id " _
                       & "  INNER JOIN invc_mstr on invc_pt_id=ps_pt_bom_id " _
                       & "  INNER JOIN si_mstr on si_id=invc_si_id " _
                       & "  inner join invct_table on invct_pt_id = invc_pt_id and invct_si_id = invc_si_id " _
                       & "  INNER JOIN public.wo_mstr ON (public.ps_mstr.ps_id = public.wo_mstr.wo_ps_id) " _
                       & "  INNER JOIN en_mstr on pt_en_id=en_id where pt_type='I'  and pt_en_id in (" & _en_id_coll & ")" _
                       & "  and wo_code = '" & _prj_code & "' "
    ...
    Private Sub fill_data()
        ...
        If fobject.name = FProductStructureAssembly.Name Or fobject.name = FProductStructurePackageAssembly.Name Then
            If _pil < 3 Then
                If _pil = 1 Then
        ...
        ElseIf fobject.name = "FProductStructure" Then
            ...
            Else
                ...
                fobject.gv_edit.SetRowCellValue(_row, "psd_indirect", "N")
                fobject.gv_edit.SetRowCellValue(_row, "pt_desc", ds.Tables(0).Rows(_row_gv).Item("descptbom"))
                fobject.gv_edit.SetRowCellValue(_row, "psd_pt_um", ds.Tables(0).Rows(_row_gv).Item("pt_um"))
                fobject.gv_edit.SetRowCellValue(_row, "um_desc", ds.Tables(0).Rows(_row_gv).Item("um_desc"))
                'fobject.gv_edit.SetRowCellValue(_row, "psd_cost", ds.Tables(0).Rows(_row_gv).Item("invct_cost"))
                fobject.gv_edit.SetRowCellValue(_row, "psd_cost", ds.Tables(0).Rows(_row_gv).Item("invct_cost"))
                fobject.gv_edit.SetRowCellValue(_row, "psd_real_cost", ds.Tables(0).Rows(_row_gv).Item("invct_cost"))
                fobject.gv_edit.BestFitColumns()
            End If
