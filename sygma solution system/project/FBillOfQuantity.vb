...
#Region "DML"
    ...
    Public Overrides Function insert() As Boolean
        ...
        _boq_code = func_coll.get_transaction_number("BQ", boq_en_id.GetColumnValue("en_code"), "boq_mstr", "boq_code")
                ...
                With objinsert
                    ...
                    Try
                        ...
                        For i = 0 To gv_stand_edit.RowCount - 1
                            ...
                            If find_pt_id(dt_stand, "boqs_pt_id", gv_stand_edit.GetRowCellValue(i, "boqs_pt_id"), "boqs_is_manual", gv_stand_edit.GetRowCellValue(i, "boqs_is_manual")) = True Then
                                .Command.CommandType = CommandType.Text
                                .Command.CommandText = "UPDATE  " _
                                                    & "  public.boqs_stand   " _
                                                    & "SET  " _
                ...
                Using objinsert As New master_new.WDABasepgsql("", "")
                    With objinsert
                        ...
                        Try
                            ...
                            .Command.CommandType = CommandType.Text
                            .Command.CommandText = "delete from boq_mstr where boq_oid='" & ds.Tables(0).Rows(BindingContext(ds.Tables(0)).Position).Item("req_oid") & "'"
                            .Command.ExecuteNonQuery()
                            .Command.Parameters.Clear()
...
#End Region
    ...
    Private Sub btRetriveDetail_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btRetriveDetail.Click
        Try
            Dim ssql As String
            ssql = "SELECT  " _
                & "  a.prjd_pt_id as boqd_pt_id, " _
                & "  b.pt_code, " _
                & "  b.pt_desc1, " _
                & "  b.pt_desc2, " _
                & "  a.prjd_um as boqd_um, " _
                & "  a.prjd_qty as boqd_qty_plan, " _
                & "  c.code_code,a.prjd_oid as boqd_sopjd_oid,b.pt_pm_code " _
                & "FROM " _
                & "  public.prjd_det a " _
                & "  INNER JOIN public.pt_mstr b ON (a.prjd_pt_id = b.pt_id) " _
                & "  INNER JOIN public.code_mstr c ON (a.prjd_um = c.code_id) " _
                & "WHERE " _
                & "  a.prjd_prj_oid = '" & boq_sopj_oid.Tag & "' " _
                & " ORDER by prjd_seq "
    ...
    Private Sub btRetriveStandar_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btRetriveStandar.Click
        Try
            ...
            For i As Integer = 0 To gv_edit.RowCount - 1
                With gv_edit
                    If .GetRowCellValue(i, "pt_pm_code").ToString.ToUpper <> "P" Then
                        ssql += "SELECT  psd_pt_bom_id as boqs_pt_id,psd_comp as pt_code,   " _
                             ...
                             & " from public.get_all_simulated('" & .GetRowCellValue(i, "pt_code") & "',  " _
                             & SetNumber(.GetRowCellValue(i, "boqd_qty_plan")) & ", " & "'Y'" & "," _
                             & SetNumber(boq_en_id.EditValue) & ",'Y', CURRENT_DATE) "
                    Else
                        ssql += "select " & .GetRowCellValue(i, "boqd_pt_id") & " as boqs_pt_id,'" & .GetRowCellValue(i, "pt_code") _
                            & "' as pt_code,'" & .GetRowCellValue(i, "pt_desc1") & "' as pt_desc1,'" & .GetRowCellValue(i, "pt_desc2") & "' as pt_desc2,'" _
                            & .GetRowCellValue(i, "code_code") & "' as code_code," & .GetRowCellValue(i, "boqd_um") & " as pt_um," _
                            & .GetRowCellValue(i, "boqd_qty_plan") & " as boqs_qty_plan,cast('N' as CHAR) as boqs_is_manual"
                    End If
                  

                    If i <> gv_edit.RowCount - 1 Then
                        ssql += " UNION "
                    End If
            ...
            ssql = "SELECT boqs_pt_id,pt_code,pt_desc1,pt_desc2,code_code,boqs_is_manual,sum(boqs_qty_plan) as boqs_qty_plan from (" & ssql _
                & ") as temp group by boqs_pt_id,pt_code,pt_desc1,pt_desc2,code_code,boqs_is_manual"
